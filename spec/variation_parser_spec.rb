require 'share_progress'
require 'share_progress/twitter_variant'
require 'share_progress/facebook_variant'
require 'share_progress/email_variant'
require 'share_progress/variant_parser'

module ShareProgress
  describe VariantParser do
    let(:facebook_type) { FacebookVariant }
    let(:facebook_hash) { {facebook_title: 'title'} }
    let(:twitter_type) { TwitterVariant }
    let(:twitter_hash) { {twitter_message: 'message'} }
    let(:email_type) { EmailVariant }
    let(:email_hash) { {email_subject: 'subject'} }

    it 'can correctly identify a facebook hash' do
      expect(VariantParser.parse(facebook_hash)).to eq(facebook_type)
    end

    it 'can correctly identify a twitter hash' do
      expect(VariantParser.parse(twitter_hash)).to eq(twitter_type)
    end

    it 'can correctly identify an email hash' do
      expect(VariantParser.parse(email_hash)).to eq(email_type)
    end

    it 'returns nil on an invalid hash' do
      expect(VariantParser.parse({bad_key: 'bad val'})).to eq(nil)
    end

    it 'returns nil on an empty hash' do
      expect(VariantParser.parse({})).to eq(nil)
    end
  end
end
